...
(function(){
  window.loadWeather=async function(loc){
    ...
    return{
      ...
      hourly:h.time.map((t,i)=>({
        ...
        precipitationChance:(h.precipitation_probability[i]||0)/100,
        precipitationAmount:h.precipitation[i],
      ...
      daily:d.time.map((t,i)=>({
        ...
        temperatureApparentMax:d.apparent_temperature_max[i],
        temperatureApparentMin:d.apparent_temperature_min[i],
        weather_code:d.weather_code[i],
        sunrise:d.sunrise[i],
        sunset:d.sunset[i],
        uvIndexMax:d.uv_index_max[i],
        precipitationChance:(d.precipitation_probability_max[i]||0)/100,
        precipitationAmount:d.precipitation_sum[i],
  ...
  window.loadTides=async function(loc){
    ...
    if(!r.ok)throw new Error(x.error||'WorldTides data is unavailable. Check WORLD_TIDES_API_KEY in Vercel.');

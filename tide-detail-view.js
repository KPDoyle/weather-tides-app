(function(){
  ...
  window.drawTideChart=function(activeIndex=null){
    ...
    for(let i=0;i<6;i++){const y=T+i*H/5,val=hi-i*range/5;ctx.beginPath();ctx.moveTo(L,y);ctx.lineTo(cssWidth-R,y);ctx.stroke();ctx.textAlign='right';ctx.textBaseline='middle';ctx.fillText(`${val.toFixed(2)} ${unit()}`,L-10,y)}
    ...
    for(let ts=firstTick;ts<=t1;ts+=tickHours*3600000){
      const x=L+(ts-t0)*W/span;
      if(x-L<58||cssWidth-R-x<58)continue;
      const d=new Date(ts);

(() => {
  ...
  function showNormalDashboard(scroll=false){
    document.querySelectorAll('.tab').forEach(b=>{const active=b.dataset.tab==='home';b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active))});
    Object.values(panelIds).forEach(id=>{const panel=document.getElementById(id);if(panel)panel.hidden=true});
    if(scroll)(document.querySelector('.reference-overview')||document.getElementById('dashboard'))?.scrollIntoView({behavior:'smooth',block:'start'});
  }
  window.showNormalDashboard=showNormalDashboard;
  function switchTab(name){
    if(name==='home'){showNormalDashboard(true);return}
  ...
  if(brand){brand.setAttribute('role','button');brand.setAttribute('tabindex','0');brand.setAttribute('aria-label','Return to normal dashboard');brand.addEventListener('click',()=>showNormalDashboard(true));brand.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();showNormalDashboard(true)}})}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>showNormalDashboard(false),{once:true});else showNormalDashboard(false);
